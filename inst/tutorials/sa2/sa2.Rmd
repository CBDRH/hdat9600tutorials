...
This follows the same logic as the Cox model with step function illustrated in the previous section. the `pchreg` function from the `eha` package can be used to fit proportional hazards models with piecewise constant baseline hazard(s). note the `cuts =` argument used to specify the the points in time where the hazard function jumps.
...
pch10.mv1 <- pchreg(Y ~ sex + civ + region, data = oldmort, cuts = c(10, 20, 30))
pch10.mv1

# fitting a piecewise constant hazards distribution with eight 5-year intervals
pch5.mv1 <- pchreg(Y ~ sex + civ + region, data = oldmort, cuts = c(5, 10, 15, 20, 25, 30, 35))
pch5.mv1

# fitting a piecewise constant hazards distribution with ten 4-year intervals
pch4.mv1 <- pchreg(Y ~ sex + civ + region, data = oldmort, cuts = c(4, 8, 12, 16, 20, 24, 28, 32, 36))
pch4.mv1
